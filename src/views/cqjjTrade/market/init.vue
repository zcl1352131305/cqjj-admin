    ...
    <div class="filter-container">
      <el-form :inline="true" :model="searchForm" class="demo-form-inline">
        ...
        <el-form-item>
          ...
          <router-link :to="{ path: 'add' }" v-if="auths.indexOf('cqjjTradeMarket:btn_add') != -1">
        ...
        <el-table-column label="操作" width="180">
          <template scope="props">
            <router-link :to="{path: 'edit/'+ props.row.id}" tag="span">
              <el-button type="info" size="small" icon="edit"  v-if="auths.indexOf('cqjjTradeMarket:btn_upd') != -1">修改</el-button>
            </router-link>
            <el-button type="danger" size="small" icon="delete" @click="delData(props.row.id)" v-if="auths.indexOf('cqjjTradeMarket:btn_del') != -1">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-row class="mt-5">
        <el-col :span="24">
          <el-button class="fl" type="danger" size="small" icon="delete" :disabled="batchSelect.length === 0" @click="batchDelData" v-if="auths.indexOf('cqjjTradeMarket:btn_del') != -1">批量删除</el-button>
    ...
    methods: {
      ...
      delData(id) {
        this.$confirm('确认删除选中记录吗？', '提示', {
          ...
          this.$store.dispatch('doDelete', {
            url: '/cqjjTrade/market/remove/' + id
          }).then((data) => {
            this.$message({
              message: '删除成功',
              type: 'success'
      ...
      batchDelData() {
        this.$confirm('确认删除选中记录吗？', '提示', {
          ...
          this.$store.dispatch('doDelete', {
            url: '/cqjjTrade/market/batchRemove/' + ids
          }).then((data) => {
            this.$message({
              message: '删除成功',
              type: 'success'
